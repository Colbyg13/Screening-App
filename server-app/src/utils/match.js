const invokeIfCallback = (cb, ...args) => (typeof cb === 'function' ? cb(...args) : cb)
...
const match = (...inputs) => ({
    on: (predicate, cb) =>
        predicate(...inputs) ? matched(invokeIfCallback(cb, ...inputs)) : match(...inputs),
    case: (condition, cb) =>
        condition ? matched(invokeIfCallback(cb, ...inputs)) : match(...inputs),
    equals: (...args) =>
        inputs.length === 1
            ? inputs[0] === args[0]
            ...
            : inputs.every((input, i) => input === args[i])
              ...
              : match(...inputs),
    in: (array, cb) =>
        inputs.every(input => array.includes(input))
    ...
    against: obj =>
        Object.entries(obj).reduce(
            (acc, [key, cb]) =>
                acc.case(`${inputs[0]}` === key, () => invokeIfCallback(cb, ...inputs.slice(1))),
