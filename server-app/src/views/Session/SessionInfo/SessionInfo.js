...
export default function SessionInfo() {
    ...
    return (
        <div className="flex h-screen">
            <OfflineIdModal offlineId={offlineId} onClose={() => setOfflineId()} />
            <StationInfoList />
            <SessionInfoConsole />
            <div className="absolute bottom-4 right-4 space-x-4 flex">
                <Button size="large" color="primary" variant="contained" onClick={getNextOfflineId}>
                    Next Offline ID
                </Button>
                <Button size="large" color="error" variant="contained" onClick={stopSession}>
                    Finish Session
                </Button>
            </div>
        </div>
