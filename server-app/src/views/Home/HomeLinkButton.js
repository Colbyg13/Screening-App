...
export default function HomeLinkButton({
    ...
    const colorClassName = to === ROUTES.Records.path ?
    ...
    return (
        <Link className={`h-40 flex-1 flex items-center justify-center cursor-pointer rounded-lg shadow-lg ${colorClassName}`}
            to={to}
        >
            <h3 className='text-md sm:text-xl md:text-3xl lg:text-4xl text-white text-center'>{title}</h3>
        </Link>
    )
}
