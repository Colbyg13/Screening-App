...
const fs = require('fs');
const { SEQUENCE_NAMES, resetSequenceValue } = require('./database/utils/idCounters');
        ...
        deleteAllRecordsAndSessions: () =>
            new Promise(async (res, rej) => {
                ...
                try {
                    ...
                    // reset the record ids
                    await resetSequenceValue(SEQUENCE_NAMES.RECORDS);
        ...
        downloadRecords: (initialOutputPath, allFieldKeys = [], unitConversions = {}) =>
            new Promise((resolve, reject) => {
                ...
                writeStream.write(
                    `${allFieldKeys
                        .map(key =>
                            unitConversions[key] ? `${key} (${unitConversions[key]})` : key,
                        )
                        .join(',')}\n`,
                );
