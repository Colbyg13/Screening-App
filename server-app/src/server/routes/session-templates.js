...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: 'Could not get session templates due to Internal Server Error',
        });
    }
});
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: 'Could not create session templates due to Internal Server Error',
        });
    }
});
...
router.delete('/:templateId', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: 'Could not delete session template due to Internal Server Error',
        });
    }
});
